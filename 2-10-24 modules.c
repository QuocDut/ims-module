...
typedef struct 
{
	char phone_number[100][20];
	char user_name[100][20];
	char ip[100][100];
	char port[100][5];
	int index;
} User_Memory;
...
User_Memory* init_shared_memory()
{
    ...
    shm = mmap(NULL, sizeof(User_Memory), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (shm == MAP_FAILED) {
        perror("mmap failed");
        close(shm_fd);
        if (created) {
...
void send_sip_message(int sockfd, struct sockaddr_in *client_addr,char* method ,SIP_Headers* headers, SDP* sdp ) {
	...
	if (strcmp(method, "401 Unauthorized") == 0) {
        strncat(response, "WWW-Authenticate: Digest realm=\"ss7demo.lab.cirpack.com\"," "nonce=\"00000acd29b0016738ed40e93cf545c6\"," "opaque=\"00000ac74f2d2ff\",stale=false,algorithm=MD5\r\n", 
